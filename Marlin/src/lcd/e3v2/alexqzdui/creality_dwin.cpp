    ...
    void Draw_Bed_Mesh(int16_t selected = -1, uint8_t gridline_width = 1, uint16_t padding_x = 8, uint16_t padding_y_top = 40 + 53 - 7) {
      ...
      GRID_LOOP(x, y) {
        ...
        while (LCD_SERIAL.availableForWrite() < 32) { // wait for serial to be available without blocking and resetting the MCU 
          gcode.process_subcommands_now(F("G4 P10"));
          planner.synchronize();
        } 
        // Draw value text on 
        if (viewer_print_value) { 
          gcode.process_subcommands_now(F("G4 P10"));  // still fails without additional delay...
          planner.synchronize();
...
void CrealityDWINClass::Draw_Menu_Item(uint8_t row, uint8_t icon/*=0*/, const char * label1, const char * label2, bool more/*=false*/, bool centered/*=false*/, bool onlyCachedFileIcon/*=false*/) {
  ...
  #ifdef DWIN_CREALITY_LCD_ALEXQZDUI_GCODE_PREVIEW
...
void CrealityDWINClass::Menu_Item_Handler(uint8_t menu, uint8_t item, bool draw/*=true*/) {
  ...
  switch (menu) {
    ...
    case Visual:
      ...
      #define VISUAL_FILE_TUMBNAILS (VISUAL_COLOR_THEMES + ENABLED(DWIN_CREALITY_LCD_ALEXQZDUI_GCODE_PREVIEW))
        ...
        if (paused) {
          if (sdprint) {
            wait_for_user = false;
              ...
              gcode.process_subcommands_now(PSTR(cmnd));
